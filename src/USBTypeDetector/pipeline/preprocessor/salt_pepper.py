...
def detect_salt_pepper_noise(image, kernel_size=3, diff_threshold=55, min_fraction=0.01):
...
def fix_salt_pepper(image, noise_type, kernel_size=Config.cfg["kernel_size_salt_pepper"]):
    kernel = np.ones((kernel_size, kernel_size), np.uint8)
    if noise_type.lower() == "salt":
        fixed = cv2.erode(image, kernel)            # Minimum filter removes bright (salt) noise.
    elif noise_type.lower() == "pepper":
        fixed = cv2.dilate(image, kernel)           # Maximum filter removes dark (pepper) noise.
...
def run_salt_pepper(image, kernel_size=Config.cfg["kernel_size_salt_pepper"], diff_threshold=Config.cfg["diff_threshold"], min_fraction=Config.cfg["min_fraction"]):
    print(f"-- Detecting salt and pepper noise with kernel size: {kernel_size}, diff threshold: {diff_threshold}, min fraction: {min_fraction} --")
    noise_type = detect_salt_pepper_noise(image, kernel_size, diff_threshold, min_fraction)
